...
- **Assets service:** Points to `us-assets.i.posthog.com` or `eu-assets.i.posthog.com` for the JavaScript SDK, remote config, and other static files
...
metadata:
  ...
  annotations:
    ...
    nginx.ingress.kubernetes.io/proxy-set-headers: default/posthog-proxy-headers
spec:
  tls:
  - hosts:
    - e.yourdomain.com
...
- `default/posthog-proxy-headers`: Change `default` if you deploy these resources in another namespace
...
- `proxy-set-headers` removes browser cookies and unnecessary forwarding metadata before sending requests to PostHog. ingress-nginx continues to send `X-Forwarded-For`, which preserves client IP addresses for geolocation.
...
3. Verify your Ingress has `/static` and `/array` path rules before the catch-all `/` rule. Ingress path matching is order-dependent.
